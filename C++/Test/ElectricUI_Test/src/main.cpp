...
float Moment_Inertia; float mass = 10; float radius = 2;
...
void loop() {
  ...
  */
  ...
  Roll_Old = Roll + ((YGyro*(180/M_PI)) * dt);
  Pitch_Old = Pitch + ((XGyro*(180/M_PI)) * dt);
  Yaw = Yaw + ((ZGyro*(180/M_PI)) * dt);
    //Complementary filter, not for Yaw b/c doesn't have one
  Roll = 0.98 * Roll_Old + 0.02 * YAccel;
  Pitch = 0.98 * Pitch_Old + 0.02 * XAccel;
  /*
  //Serial.print(" Roll: ");
  Serial.print(Roll,3); 
  Serial.print("\t");
  ...
  */
  ...
  //Thrust calculations
  Moment_Inertia = (1/2)*mass*(radius*radius);
  Angular_Moment = (abs(ZGyro)*Moment_Inertia)/2;
